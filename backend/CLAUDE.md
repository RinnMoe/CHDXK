...
No Makefile or CI config exists yet. Config template: `config/config.example.yaml` — copy to `config/config.yaml` for local runs. Any config value can be overridden by env vars with `JCOURSE_` prefix (e.g. `JCOURSE_SERVER_ADDR=:9090`). SQL schema lives in `script/01-schema-pgsql.sql`.
